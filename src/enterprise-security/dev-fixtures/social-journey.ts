...
export function socialAuthorizationUri(provider: SocialProviderV1): string {
  ...
  for (const [key, value] of Object.entries({
    client_id: 'development-only',
    redirect_uri: 'https://identity.example.invalid/api/auth/v1/security/identity/social/callback',
    response_type: 'code',
    scope: provider === 'github' ? 'user:email' : 'openid email',
    state: 'A'.repeat(43),
