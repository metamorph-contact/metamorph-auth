...
import { assertSocialStartDestination } from './federation-client'
...
function setup(provider: SocialProviderV1, transport?: FederationTransport) {
  ...
  const result = {
    ...
    attemptId: id,
    provider,
    redirectRegistrationId: 'local-a-social',
    navigationUri: socialAuthorizationUri(provider),
...
describe.each(providers)('%s typed social entry', (provider) => {
  ...
  it('shows typed policy denial and retains the exact command across provider outage', async () => {
    ...
    const { resolution, calls, journey } = setup(provider, async () => ({
      ...
      body: JSON.stringify({
        ...
        correlationId: id,
        error: {
          code: outage ? 'security.provider.unavailable' : 'security.method.disabled',
        },
      }),
    }))
  ...
  it('never starts a hidden choice and rejects endpoint/scope/token/duplicate mixup', async () => {
    ...
    for (const mutate of [
      (u: URL) => {
        u.hostname = 'attacker.example'
      },
      (u: URL) => {
        u.searchParams.set('scope', 'repo mail calendar')
      },
      (u: URL) => {
        u.searchParams.set('access_token', 'never-retained')
      },
      (u: URL) => {
        u.searchParams.append('state', 'D'.repeat(43))
      },
      (u: URL) => {
        u.pathname += '/other'
      },
    ]) {
      const u = new URL(result.navigationUri)
      mutate(u)
  ...
  it('binds the start result to the selected identity callback and registration', () => {
    const { result } = setup(provider)
    expect(() => assertSocialStartDestination(result, 'https://identity.example.invalid')).not.toThrow()
    const navigation = new URL(result.navigationUri)
    navigation.searchParams.set('redirect_uri', 'https://attacker.example/api/auth/v1/security/identity/social/callback')
    expect(() => assertSocialStartDestination({ ...result, navigationUri: navigation.href }, 'https://identity.example.invalid')).toThrow('security.ceremony.mismatch')
  })
  it('accepts the exact loopback callback used by development catalogs', () => {
    const { result } = setup(provider)
    const identityOrigin = 'http://local-a.api.identity.localhost:1424'
    const navigation = new URL(result.navigationUri)
    navigation.searchParams.set('redirect_uri', `${identityOrigin}/api/auth/v1/security/identity/social/callback`)
    const loopbackResult = { ...result, navigationUri: navigation.href }
    expect(() => decodeFederationResponse('identity.social.start', JSON.stringify(loopbackResult))).not.toThrow()
    expect(() => assertSocialStartDestination(loopbackResult, identityOrigin)).not.toThrow()
  })
})
it('keeps one original command after loss, refuses another provider, and erases it on scope exit', async () => {
  const { resolution, journey, calls } = setup('google', async () => {
