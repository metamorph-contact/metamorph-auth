...
const { clients, clearSocial } = vi.hoisted(() => ({
  clients: vi.fn(),
  clearSocial: vi.fn(),
}))
...
vi.mock('../protocol/client', async (original) => ({
  ...(await original<Record<string, unknown>>()),
  clearRealmSocialAuthorization: clearSocial,
}))
...
describe.each(['google', 'microsoft', 'github'] as const)('%s common social choices', (provider) => {
  it('is offered without email and displays a typed denial, with no double start', async () => {
    ...
    const call = vi.fn(async (op: string) => {
      ...
      throw new FederationHttpError(403, {
        schemaVersion: 1,
        correlationId: resolution.continuationId,
    ...
    const flow = {
      bootstrap: {
        flowId: resolution.continuationId,
    ...
    const button = await screen.findByRole('button', {
      name: new RegExp(`Continue with ${provider}`, 'iu'),
    })
...
it('scrubs a pending social start on page suspension and never navigates a late response', async () => {
  const resolution = federationJourneyFixture().resolution
  resolution.methods.push('social')
  resolution.socialProviders = ['github']
  let release!: () => void
  const call = vi.fn(async (op: string) => {
    if (op === 'identity.methods.resolve') return resolution
    ...
    return {
      schemaVersion: 1,
      attemptId: resolution.continuationId,
      provider: 'github',
      redirectRegistrationId: 'local-a-social',
      navigationUri: socialAuthorizationUri('github'),
      expiresAt: resolution.expiresAt,
    }
  })
  clients.mockImplementation(() => ({ call }))
  ...
  const call = vi.fn(async (op: string) =>
    op === 'identity.methods.resolve'
      ...
      : {
          ...
          provider: 'google',
          redirectRegistrationId: 'local-a-social',
          navigationUri: socialAuthorizationUri('google'),
          expiresAt: expiry,
        },
  )
  clients.mockImplementation(() => ({ call }))
...
it('uses the newly selected provider region after a definite start denial', async () => {
  ...
  const flow = {
    bootstrap: {
      flowId: '01994000-0000-7000-8000-000000000010',
      expiresAt: resolution.expiresAt,
    },
    catalog: {
      projection: {
  ...
  render(<FederationMethodChoices flow={flow as never} email="chosen@example.com" disabled={false} navigate={vi.fn()} />)
